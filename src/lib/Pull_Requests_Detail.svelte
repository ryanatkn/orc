<script lang="ts">
	...
	export let packages: Fetched_Package[];

	// TODO BLOCK unfinished
	interface Filter_Pull_Request {
		(pull: Github_Pull_Request, pkg: Fetched_Package): boolean;
	}
	...
	// TODO BLOCK type is wrong for `packages` - just adds `pulls` to `url` and `package_json`
		...
		{#each pull_requests as pull_request}
			{#if pull_request.pkg.repo_url}
				<tr>
					...
					>
					<td
						><a href={pull_request.pull.url} title={pull_request.pull.title}
							>#{pull_request.pull.number}</a
						></td
					>
